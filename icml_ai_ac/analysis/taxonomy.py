...
def collect_cases(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, direction in (("overlooked_gems", "overlooked_gem"), ("blind_spots", "blind_spot")):
        ...
        for card in section.get("cases", []) if isinstance(section.get("cases"), list) else []:
...
def balanced_sample(cases: list[dict[str, Any]], sample_size: int) -> list[dict[str, Any]]:
    ...
    sampled = gems[:half] + blind[:half]
    return sampled[:sample_size] if sample_size else sampled
...
def case_brief(case: dict[str, Any]) -> str:
    ...
    lines = [
        f"PAPER {case['paper_id']} | direction={case['direction']} | class={case.get('contribution_class')}",
        f"  Human: tier={human.get('tier')} award={human.get('is_award')} "
        f"reviewer_overall={human.get('reviewer_overall')} soundness={human.get('reviewer_soundness')}",
        f"  AI: executive_priority={ai.get('executive_ac_priority')} percentile={ai.get('percentile')} "
...
Propose a compact codebook of reason codes describing WHY the AI's ranking diverged from the human outcome. Ground each code in the observable contrast (which AI axes are high or low relative to the human tier and reviewer scores) — for example the AI rewarding sweeping-impact framing while reviewers rewarded soundness. Do NOT simply restate the AI's self-justification. Codes must be distinct and reusable.
...
def validate_codebook(payload: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    errors: list[str] = []
...
def run_taxonomy_induction(
    *,
    ...
    if result["status"] != "ok":
        ...
        return {**base, **result}
    codes, errors = validate_codebook(result["parsed"])
    payload = {
        **base,
...
Assign codes by grounding them in the observable contrast between the AI's axis scores and the human outcome (tier, reviewer soundness/overall), not by echoing the AI's own justification. Use only codebook codes. Prefer 1-2 codes; add a third only if clearly warranted.
    ...
    for row in rows:
        ...
        for entry in row.get("codes", []) if isinstance(row.get("codes"), list) else []:
            ...
            if code not in code_names:
                ...
                continue
            assigned.append({"code": code, "evidence": str((entry or {}).get("evidence") or "") if isinstance(entry, dict) else ""})
        by_id[paper_id] = [item["code"] for item in assigned]
    assignments = [
        {"paper_id": paper_id, "codes": by_id.get(paper_id, [])}
...
def classify_cases(
    ...
    *,
    ...
    for index in range(0, len(cases), max(1, batch_size)):
        ...
        result = _complete_json(client, messages, config)
        batch_meta.append({"batch_start": index, "status": result["status"]})
        if result["status"] != "ok":
            errors.append(f"batch {index}: {result.get('error') or result['status']}")
            for case in batch:
                assignments.append({"paper_id": case["paper_id"], "codes": []})
            continue
...
def run_taxonomy_classification(
    *,
    ...
    if second_client is not None:
        second_assignments, second_errors, _ = classify_cases(
            cases, codebook, config=config, client=second_client, batch_size=batch_size,
...
def cohens_kappa_per_code(
    ...
    a_by_id = {row["paper_id"]: set(row.get("codes", [])) for row in assignments_a}
    b_by_id = {row["paper_id"]: set(row.get("codes", [])) for row in assignments_b}
    ...
    for code in code_names:
        ...
        result[code] = cohens_kappa(a_labels, b_labels)
    return result
